 ...
 *     // -or-
 *     return res.HandleError(optionalData);
 * ```
 *
 * Or with actions2:
 * ```
...
module.exports = function HandleError(optionalData) {
  ...
  if (optionalData === undefined) {
    sails.log.info('Ran custom response: res.HandleError()');
  ...
  // except in production use res.sendStatus().
    ...
    sails.log.info('Custom response `res.HandleError()` called with an Error:', optionalData);
